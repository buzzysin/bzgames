...
template <typename... Args> void bz_ignored(Args &&.../*unused*/) {}
...
// Detect DLL Support (defines BZ_API <=> dllimport/export)
#if BZ_PLATFORM_(WINDOWS)
	#if defined(BZ_BUILD_DLL)
		#define BZ_API BZ_EXPORT
	#else
		#define BZ_API BZ_IMPORT
	#endif
#else
	#define BZ_API
#endif
